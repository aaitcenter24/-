  ...
  return (
    <div className="fixed inset-0 z-[600] flex items-center justify-center p-4">
      ...
      <motion.div 
        ...
        className="relative w-full max-w-md bg-white rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col max-h-[90dvh]"
      >
        <div className="bg-emerald-600 p-6 text-white shrink-0 relative overflow-hidden">
          ...
          <div className="flex items-center justify-between relative z-10">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] font-bold text-emerald-100 uppercase tracking-widest opacity-80">
                  {type === 'land' 
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
          <div className="space-y-3">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">{t.inputLabel}</label>
            <div className="relative">
               <input 
                 ...
                 className="w-full px-6 py-5 bg-slate-50 border-2 border-slate-100 rounded-3xl text-2xl font-black text-emerald-800 outline-none focus:border-emerald-500 focus:bg-white transition-all shadow-inner"
               />
          ...
          <div className="space-y-3">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">{t.selectLabel}</label>
          ...
          <div className="space-y-3">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">{t.conversionsLabel}</label>
            <div className="bg-slate-50 rounded-[2rem] p-4 border-2 border-slate-100 border-dashed space-y-4">
              <div className="grid grid-cols-2 gap-x-4 gap-y-3">
                {units.map(unit => {
                  const val = baseValue / unit.ratio;
                  const isAppMatch = unit.id === appUnit.id;
                  return (
                    <div key={unit.id} className={`p-3 rounded-2xl transition-all ${isAppMatch ? 'bg-emerald-100/50 ring-2 ring-emerald-500/20' : ''}`}>
                      <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest block mb-0.5">
                        {lang === 'bn' ? unit.labelBn : lang === 'ar' ? unit.labelAr : unit.labelEn}
                      </span>
                      <span className={`text-sm font-black truncate block ${unit.id === selectedUnitId ? 'text-emerald-500' : 'text-slate-700'}`}>
          ...
          <div className="flex flex-col gap-3">
            <div className="flex items-center justify-between px-2">
              <div className="flex flex-col">
                <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{t.targetPrefix}</span>
            ...
            <button 
              ...
              className="w-full py-5 rounded-2xl bg-emerald-600 text-white text-xs font-black uppercase tracking-[0.2em] shadow-xl shadow-emerald-200 hover:bg-emerald-700 active:scale-95 transition-all disabled:opacity-20 disabled:grayscale disabled:pointer-events-none flex items-center justify-center gap-3"
            >
              <RefreshCw size={18} />
              {t.apply}
            </button>
