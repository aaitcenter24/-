  ...
  return (
    <div className="fixed inset-0 z-[700] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
      <motion.div 
        ...
        dir={isRtl ? 'rtl' : 'ltr'}
        className="relative w-full max-w-4xl bg-white rounded-[3rem] shadow-2xl overflow-hidden flex flex-col md:flex-row max-h-[90dvh]"
      >
        {/* Visual Sidebar (Desktop Only) */}
        <div className="hidden md:flex w-1/3 bg-emerald-600 p-12 text-white flex-col justify-between relative overflow-hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 flex-1 mb-8">
            ...
            >
              <div className="space-y-6">
                ...
                <div>
                  <h4 className="text-xl font-black text-slate-900 uppercase tracking-tight">{t.free.name}</h4>
                  <p className="text-2xl font-bold text-slate-700 mt-1">{t.free.price}</p>
                </div>
                <ul className="space-y-3">
                  {t.free.features.map((feature: string, idx: number) => (
                    <li key={idx} className="flex items-start gap-3">
                      <div className="mt-1 shrink-0 w-4 h-4 rounded-full bg-slate-200 flex items-center justify-center">
                        <Check size={10} className="text-slate-500" />
                      </div>
                      <span className="text-xs font-bold text-slate-500 leading-tight">{feature}</span>
                    </li>
                  ))}
                </ul>
              </div>
            ...
            >
              ...
              <div className="space-y-6">
                <div className="flex justify-between items-center relative z-10">
                  <div className={`w-12 h-12 rounded-2xl shadow-sm flex items-center justify-center ${
                    currentPlan === 'pro' ? 'bg-white text-emerald-600' : 'bg-emerald-500 text-white'
                  }`}>
                    <Crown size={24} />
                  </div>
                  {currentPlan === 'pro' && (
                    <span className="px-3 py-1 bg-white text-emerald-600 text-[10px] font-black uppercase tracking-widest rounded-full">
                      {t.pro.current}
                    </span>
                  )}
                </div>
                <div className="relative z-10">
                  ...
                  <p className={`text-2xl font-bold mt-1 ${currentPlan === 'pro' ? 'text-emerald-700' : 'text-emerald-50'}`}>
                    {t.pro.price}
                  </p>
                </div>
                  ...
                  {t.pro.features.map((feature: string, idx: number) => (
                    <li key={idx} className="flex items-start gap-3">
                      <div className={`mt-1 shrink-0 w-4 h-4 rounded-full flex items-center justify-center ${
                        currentPlan === 'pro' ? 'bg-emerald-200' : 'bg-white/20'
                      }`}>
                        <Check size={10} className={currentPlan === 'pro' ? 'text-emerald-600' : 'text-white'} />
                      </div>
                      <span className={`text-xs font-bold leading-tight ${currentPlan === 'pro' ? 'text-slate-600' : 'text-emerald-50'}`}>
                        {feature}
                      </span>
                    </li>
                  ))}
              ...
              <button 
                ...
                className={`relative z-10 mt-8 w-full py-4 rounded-2xl text-xs font-black uppercase tracking-widest transition-all active:scale-95 flex items-center justify-center gap-2 ${
                  ...
                  : 'bg-white text-emerald-600 shadow-lg shadow-emerald-900/20 hover:bg-emerald-50'
                }`}
              >
